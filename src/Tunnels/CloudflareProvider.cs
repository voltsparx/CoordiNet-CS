...
public sealed class CloudflareProvider : ITunnelProvider
{
    private Process? _process;

    public string ProviderName => "cloudflared";
    ...
    public async Task<string?> StartAsync(int localPort, CancellationToken cancellationToken = default)
    {
        ...
        var binaryName = GetBinaryName("cloudflared");
        var startInfo = CreateProcessStartInfo(binaryName, $"tunnel --url http://localhost:{localPort}");
    ...
    private static string? ExtractUrl(string output)
    {
        var match = System.Text.RegularExpressions.Regex.Match(
            output,
            @"https?://[a-zA-Z0-9.-]+(?:\.trycloudflare\.com|\.workers\.dev)");
