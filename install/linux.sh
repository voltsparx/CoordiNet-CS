...
printf '\033[0;35m'
printf '========================================\n'
printf '  CoordiNet-CS Installation Flow\n'
printf '========================================\n'
printf '\033[0m\n'
...
ensure_tool() {
  ...
  read -r -p 'Install required C# toolchain now? [Y/n]: ' answer
  answer=${answer:-Y}
  if [[ "$answer" =~ ^[Yy]$ ]]; then
    if command -v apt-get >/dev/null 2>&1; then
      sudo apt-get update && sudo apt-get install -y mono-devel || sudo apt-get install -y dotnet-sdk-8.0
      ...
      printf '\033[0;31m[ERROR] No supported system package manager was detected.\033[0m\n'
      exit 1
    fi
  else
  ...
  case "$choice" in
    1)
      ...
      if [ -f "$TARGET_BIN" ]; then
        read -r -p 'Custom install path? (leave blank for default /usr/local/bin): ' custom_path
        if [ -n "$custom_path" ]; then
          mkdir -p "$custom_path"
          install -m 755 "$TARGET_BIN" "$custom_path/$APP_NAME"
          echo "export PATH=\"\$PATH:$custom_path\"" >> "$HOME/.bashrc"
          echo "export PATH=\"\$PATH:$custom_path\"" >> "$HOME/.profile"
          printf '\033[0;32m[OK] Installed to custom path and exported to shell profile.\033[0m\n'
        else
          sudo install -m 755 "$TARGET_BIN" /usr/local/bin/$APP_NAME
          printf '\033[0;32m[OK] Installed to /usr/local/bin/%s\033[0m\n' "$APP_NAME"
        fi
      else
