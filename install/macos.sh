...
printf '\033[0;35m'
printf '========================================\n'
printf '  CoordiNet-CS macOS Installer\n'
printf '========================================\n'
printf '\033[0m\n'
...
ensure_tool() {
  ...
  read -r -p 'Install .NET SDK or Mono support now? [Y/n]: ' answer
  answer=${answer:-Y}
  if [[ "$answer" =~ ^[Yy]$ ]]; then
    if command -v brew >/dev/null 2>&1; then
      brew update && brew install --cask dotnet || brew install mono
    else
      printf '\033[0;31m[ERROR] Homebrew is required for the macOS installation path.\033[0m\n'
      exit 1
    fi
  else
  ...
  case "$choice" in
    1)
      ...
      if [ -f "$TARGET_BIN" ]; then
        read -r -p 'Custom install path? (leave blank for /usr/local/bin): ' custom_path
        if [ -n "$custom_path" ]; then
          mkdir -p "$custom_path"
          install -m 755 "$TARGET_BIN" "$custom_path/$APP_NAME"
          echo "export PATH=\"\$PATH:$custom_path\"" >> "$HOME/.zprofile"
          printf '\033[0;32m[OK] Installed to custom path and exported to zsh profile.\033[0m\n'
        else
